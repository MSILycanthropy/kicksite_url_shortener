defmodule KicksiteUrlShortenerWeb.LinkControllerTest do
  use KicksiteUrlShortenerWeb.ConnCase
  ...
  @update_attrs %{
    expires_at: ~N[2022-04-14 04:37:00],
  ...
  describe "create link" do
    test "renders link when data is valid", %{conn: conn} do
      ...
      assert %{
               "id" => ^id,
               "expires_at" => "2022-04-13T04:37:00",
               "hash" => "some hash",
               "url" => "some url"
  ...
  describe "update link" do
    ...
    test "renders link when data is valid", %{conn: conn, link: %Link{id: id} = link} do
      ...
      assert %{
               "id" => ^id,
               "expires_at" => "2022-04-14T04:37:00",
               "hash" => "some updated hash",
               "url" => "some updated url"
             } = json_response(conn, 200)["data"]
    end
